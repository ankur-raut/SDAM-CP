...
def count(request):
    ...
    customers = MonumentTickets.objects.filter(date=d,verified=True)
    monuments   = customers.values_list("monument", flat=True)
    monuments=set(monuments)
    for m in monuments:
        count=0
    ...
    return render(request,"count.html",{"counter":counter})


def display_user(request,monuments):
    startdate = date.today()-timedelta(days=7)
    enddate = date.today() + timedelta(days=7)
    print(startdate)
    print(enddate)
    customers = MonumentTickets.objects.filter(monument=monuments,date__range=[startdate, enddate])
    myfilter=DetailFilter(request.GET,queryset=customers)
    customers=myfilter.qs
    customer = MonumentTickets.objects.filter(monument=monuments)
    if customer.count()!=0:
